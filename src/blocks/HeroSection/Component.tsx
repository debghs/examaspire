...
import type { HeroSectionBlock, Media } from '@/payload-types'

export const HeroSection: React.FC<{ block: HeroSectionBlock }> = ({ block }) => {
  const { backgroundImage, content, cta } = block

  // Handle media object with type guard
  const media = backgroundImage as Media | null | undefined
  const hasValidMedia = media && typeof media.url === 'string' && typeof media.alt === 'string'
