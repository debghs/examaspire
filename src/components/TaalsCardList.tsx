...
interface TaalsCardListProps {
  taals: Taal[]
}

const TaalsCardList: React.FC<TaalsCardListProps> = ({ taals }) => {
  const [searchQuery, setSearchQuery] = useState('')

  const limit = 10

  const { currentPageItems, currentPage, totalPages, goToPage } = usePagination(
    taals.filter((taal) => taal.name.toLowerCase().includes(searchQuery.toLowerCase())),
    limit,
  )
    ...
    <div className="flex flex-col gap-4 mb-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {currentPageItems.length === 0 ? (
          ...
          currentPageItems.map((taal) => (
            <Link key={taal.slug} href={`/taal/${taal.slug}`} passHref>
              <div className="aspect-[4/3] border border-bianca-400/70 rounded-xl p-6 hover:shadow-md transition-shadow flex flex-col">
                <h3 className="text-base font-bold">{taal.name}</h3>
                <p className="text-bianca-600 text-sm">
                  {taal.matra} {taal.matra === 1 ? 'beat' : 'beats'}
                </p>
              </div>
            </Link>
          ))
